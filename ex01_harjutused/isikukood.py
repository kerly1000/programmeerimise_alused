"""
Eestis kasutatav isikukood koosneb üheteistkümnest numbrist. Tutvu isikukoodi struktuuriga (https://et.wikipedia.org/wiki/Isikukood)
ja kirjuta programm, mis analüüsib isikukoode ja väljastab võimalikult rohkem infot selle kohta (sünnikuupäev, sugu jne).
Isikukoodi käsitlege kui sümbolite kogumit ehk sõnet (kuigi see koosneb numbritest), analüüsimiseks kasutage sõneoperatsioone
(vt. käsiraamat). Kuna isikukoode on keeruline testimise ajal korduvalt sisestada, on alguses mõistlik sisestada erinevad
...
Vastavalt toodud isikukoodi tutvustavale artiklile võrdle esimest kümmet numbrit ja viimast numbrit (nn. kontrollnumbrit),
et teha selgeks, kas isikukood on üldse kehtiv. Kuna isikukoodi võtame kui sõnet, aga seal olevaid arve on vaja korrutada,
peame tegema tüübiteisenduse: sõnena oleva arvu peame teisendama täisarvuks (funktsioon "int()").
...
kuid ka sellised, mis viitavad olematule kuupäevale (algusega "3950230...", mis tähendaks 30. veebruari) vms. Vali kas
"ohutu" tee (ette on antud piirid, mis kehtivad igal juhul) või loo sisemised sõltuvused (stiilis "kui kuu on aprill,
...
def decode_id_code(id_code: str) -> None:
    if is_valid(id_code):
        ...
        print(f"Oled {gender}soost {get_date_string(birth_date)} sündinud Eesti kodanik.")
    else:
        print("Viga! Kontrolli sisestatud isikukoodi!")

if __name__ == '__main__':
    # isikukood = "60201302715"
    isikukood = ("48205280369")
    # isikukood = "31212230156"
