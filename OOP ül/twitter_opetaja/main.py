...
class Tweet:
    ...
    def __init__(self, user: str, content: str, time: float, retweets: int):
        ...
        self.hashtags = list(filter(lambda word: word.startswith("#"), content.split()))
        for tag in self.hashtags:
         Tweet.hashtags[tag] = Tweet.hashtags.get(tag, 0) +1
...
def find_fastest_growing(tweets: list) -> Tweet:
    ...
    """
    if len(tweets) == 0:
        raise ValueError(tweets, "Empty list not allowed")
    return sorted(tweets, key=lambda tweet: tweet.get_growth_rate(), reverse=True) [0]
...
def sort_by_popularity(tweets: list) -> list:
    ...
    """
    return sorted(tweets, reverse=True)
...
def filter_by_hashtag(tweets: list, hashtag: str) -> list:
    ...
    """
    return list(filter(lambda tweet: tweet, tweets))
...
def sort_hashtags_by_popularity(tweets: list) -> list:
    ...
    for tweet in tweets:
        for tag in tweet.hashtags:
            hashtags_by_count[tag] = hashtags_by_count.get(tag, 0) + tweet.retweets

    return sorted(hashtags_by_count, key=lambda  tag: (-hashtags_by_count[tag],tag))
