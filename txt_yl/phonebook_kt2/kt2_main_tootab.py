...
def phonebook_main():
    a = input("Palun vali toiming ja sisesta vastav number. 1- soovin lisada kontakti, 2- soovin leida kontakti numbrit, 3- soovin leida numbri omanikku, 4- soovin avada kogu telefoniraamatu. Soovin: ")
    if a == "1":
        add_to_phonebook(phonebook)
    if a == "2":
        find_phone_number(phonebook)
    if a == "3":
        number_belonging(phonebook)
    if a == "4":
        show_phonebook_content(phonebook)
