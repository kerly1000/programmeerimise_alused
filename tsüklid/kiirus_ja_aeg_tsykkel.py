...
varem sihtpunkti. Kui palju me aga ajas võidame? Koostage programm, mis küsib käivitamisel
läbitava vahemaa pikkust (kilomeetrites, see kehtib kogu programmitöö aja), seejärel aga
küsib kasutajalt korduvalt kiirust (km/h) ning väljastab selle põhjal korrektse lausega sõiduks
kuluva aja ja erinevuse eelmisest tulemusest. Kui kasutaja uut kiirust ei sisesta, vaid
vajutab lihtsalt sisestusklahvile, siis katkestatakse tsükkel ja tänatakse kasutajat."""

def calculate_travel_time_in_seconds(distance: int, speed: int) -> float:
    time_in_hours = distance / speed
    return time_in_hours * 3600

def convert_to_hours_minutes_seconds(time_in_seconds: float) -> tuple[int, int, float]:
...
def display_travel_time_difference(previous_travel_time: float, travel_time: float) -> None:
    hours, minutes, seconds = convert_to_hours_minutes_seconds(travel_time)
    time_string = format_time_string(hours, minutes, seconds)
    print(f"Sõidule kulub {hours} tundi, {minutes} minutit, {seconds:.2f} sekundit")
    print(f"Sõidule kulub {time_string}")

    if previous_travel_time != -1:
        difference = previous_travel_time - travel_time
        hours, minutes, seconds = convert_to_hours_minutes_seconds(abs(difference))
        time_string = format_time_string(hours, minutes, seconds)
        if difference > 0:
